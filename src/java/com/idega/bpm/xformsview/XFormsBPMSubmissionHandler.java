...
public class XFormsBPMSubmissionHandler extends AbstractConnector implements
	...
	public Map<String, Object> submit(Submission submission,
		...
		if (taskInstanceId != null) {
			...
			bpmFactory.getProcessManagerByTaskInstanceId(taskInstanceId)
			        .getTaskInstance(taskInstanceId).submit(
			            xformsViewSubmission);
			
		} else {
			
			BPMFactory bpmFactory = getBpmFactory();
			Map<String, String> parameters = xformsViewSubmission
			        .resolveParameters();
			...
			if (parameters.containsKey(ProcessConstants.START_PROCESS)) {
				...
				Logger
				        ...
				        .log(
				            Level.SEVERE,
				            "Couldn't handle submission. No action associated with the submission action. Parameters="
				                    + parameters);
			}
		}
