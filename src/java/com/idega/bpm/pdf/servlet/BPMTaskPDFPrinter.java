...
public class BPMTaskPDFPrinter implements MediaWritable{
	...
	private static final Logger logger = Logger.getLogger(BPMTaskPDFPrinter.class.getName());
	...
	public void init(HttpServletRequest req, IWContext iwc) {
		...
		if (taskIdStr != null && variableName != null){
			...
			BinaryVariable binaryVariable = getBinVar(taskInstanceId, variableName);
	...
	private BinaryVariable getBinVar(
			long taskInstanceId, String variableName) {
		
		TaskInstanceW taskInstanceW = getBpmFactory().getProcessManagerByTaskInstanceId(taskInstanceId).getTaskInstance(taskInstanceId);
		return taskInstanceW.getAttachment(variableName);
